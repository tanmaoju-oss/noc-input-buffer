//Modify add independent four-VC fixed-window throughput sweep tb, Michael Tan, 20260715
//Modify add independent four-VC 5x5 4-flit queue-knee sweep tb, Michael Tan, 20260715
`timescale 1ns / 1ps

import noc_params::*;

module tb_mesh_throughput_sweep_5x5_noxim_queue_knee_4flit_4vc #(
    ...
    parameter RATE_NUM = 20,//Modify add denser rates around the saturation knee, Michael Tan, 20260709
    ...
    bit throughput_measure_enable;//Modify delimit the exact fixed throughput measurement window, Michael Tan, 20260715
    int received_flits_in_window;//Modify count delivered flits only inside the measurement window, Michael Tan, 20260715
    int received_packets_in_window;//Modify count delivered TAIL flits only inside the measurement window, Michael Tan, 20260715
    ...
    begin
        ...
        $fclose(result_fd);
        $display("[TB_INJ_SWEEP_QUEUE] results written to throughput_results.txt");//Modify identify queue-based Noxim-style sweep log, Michael Tan, 20260701
        $finish;
    end
    ...
    task dump_output();
        $dumpfile("out.vcd");
        $dumpvars(0, tb_mesh_throughput_sweep_5x5_noxim_queue_knee_4flit_4vc);//Modify use independent four-VC 4-flit knee sweep top, Michael Tan, 20260715
    endtask
    ...
    task initialize_global();
        ...
        throughput_measure_enable = 1'b0;//Modify initialize fixed-window throughput collection, Michael Tan, 20260715
        dummy_seed_value = $urandom(SEED);

        inj_rates_permille[0] = 10;//Modify keep low-load reference point, Michael Tan, 20260709
        inj_rates_permille[1] = 30;//Modify keep low-load reference point, Michael Tan, 20260709
        inj_rates_permille[2] = 50;//Modify keep low-load reference point, Michael Tan, 20260709
        inj_rates_permille[3] = 70;//Modify keep low-load reference point, Michael Tan, 20260709
        inj_rates_permille[4] = 90;//Modify keep low-load reference point, Michael Tan, 20260709
        inj_rates_permille[5] = 100;//Modify start dense knee sampling, Michael Tan, 20260709
        inj_rates_permille[6] = 120;//Modify add dense knee sampling point, Michael Tan, 20260709
        inj_rates_permille[7] = 140;//Modify add dense knee sampling point, Michael Tan, 20260709
        inj_rates_permille[8] = 160;//Modify add dense knee sampling point, Michael Tan, 20260709
        inj_rates_permille[9] = 180;//Modify add dense knee sampling point, Michael Tan, 20260709
        inj_rates_permille[10] = 200;//Modify add dense knee sampling point, Michael Tan, 20260709
        inj_rates_permille[11] = 220;//Modify add dense knee sampling point, Michael Tan, 20260709
        inj_rates_permille[12] = 240;//Modify add dense knee sampling point, Michael Tan, 20260709
        inj_rates_permille[13] = 260;//Modify add dense knee sampling point, Michael Tan, 20260709
        inj_rates_permille[14] = 280;//Modify add dense knee sampling point, Michael Tan, 20260709
        inj_rates_permille[15] = 300;//Modify end dense knee sampling point, Michael Tan, 20260709
        inj_rates_permille[16] = 350;//Modify keep high-load reference point, Michael Tan, 20260709
        inj_rates_permille[17] = 400;//Modify keep high-load reference point, Michael Tan, 20260709
        inj_rates_permille[18] = 450;//Modify keep high-load reference point, Michael Tan, 20260709
        inj_rates_permille[19] = 500;//Modify keep high-load reference point, Michael Tan, 20260709

        clear_all_inputs();
    endtask

    task open_result_file();
        result_fd = $fopen("throughput_results.txt", "w");
        if(result_fd == 0)
        begin
            $fatal(1, "[TB_INJ_SWEEP_QUEUE] cannot open throughput_results.txt");//Modify identify queue-based Noxim-style sweep fatal, Michael Tan, 20260701
        end

        $fwrite(result_fd, "injection_rate_permille injection_rate offered_flit_rate warmup_cycles measure_cycles drain_limit_cycles drain_used_cycles measure_generated measure_enqueued measure_queue_full measure_injected measure_received received_flits_window received_packets_window throughput_flits_per_cycle_per_node_x1000000 throughput_packets_per_cycle_per_node_x1000000 max_source_queue avg_latency_cycles_x1000 error_count\n");//Modify add fixed-window normalized flit and packet throughput statistics, Michael Tan, 20260715
    endtask
    ...
    task run_one_rate(input int rate_permille);
        ...
        throughput_measure_enable = 1'b1;//Modify begin collection before the first complete measurement edge, Michael Tan, 20260715
        ...
        @(negedge clk);
        throughput_measure_enable = 1'b0;
        current_phase = PHASE_DRAIN;//Modify exclude drain traffic after the final complete measurement edge, Michael Tan, 20260715
    ...
    task reset_for_rate(input int rate_permille);
        ...
        throughput_measure_enable = 1'b0;
        received_flits_in_window = 0;
        received_packets_in_window = 0;//Modify reset fixed-window throughput counters per rate, Michael Tan, 20260715
        cycle_count = 0;
    ...
    task send_payload_flit(input int src_x, input int src_y);
        ...
        is_valid_cmd[src_x][src_y] = 1'b1;
        data_cmd[src_x][src_y] = make_flit(pkt_id, payload_label);
    ...
    task monitor_outputs();
        ...
        begin
            ...
            begin
                if(is_valid_o[x][y])
                begin
                    if(throughput_measure_enable)
                        received_flits_in_window++;//Modify count every delivered flit inside the fixed window, Michael Tan, 20260715
                    ...
                    begin
                        if(throughput_measure_enable)
                            received_packets_in_window++;//Modify count completed packets inside the fixed window, Michael Tan, 20260715
    ...
    task write_one_rate_result();
        ...
        longint throughput_denominator;
        longint flit_throughput_x1000000;
        longint packet_throughput_x1000000;
        int offered_flit_rate_permille;
        ...
        throughput_denominator = MEASURE_CYCLES_PER_RATE * MESH_SIZE_X * MESH_SIZE_Y;
        flit_throughput_x1000000 = received_flits_in_window;
        flit_throughput_x1000000 = (flit_throughput_x1000000 * 1000000) / throughput_denominator;
        packet_throughput_x1000000 = received_packets_in_window;
        packet_throughput_x1000000 = (packet_throughput_x1000000 * 1000000) / throughput_denominator;
        offered_flit_rate_permille = current_rate_permille * PACKET_FLIT_NUM;//Modify normalize fixed-window throughput per cycle and node, Michael Tan, 20260715

        $fwrite(result_fd, "%0d %0d.%03d %0d.%03d %0d %0d %0d %0d %0d %0d %0d %0d %0d %0d %0d %0d %0d %0d %0d %0d\n",//Modify align 19 output columns with 21 decimal-format arguments, Michael Tan, 20260715
                ...
                current_rate_permille % 1000,//Modify print exact decimal injection rate for low-rate points, Michael Tan, 20260709
                offered_flit_rate_permille / 1000,
                offered_flit_rate_permille % 1000,//Modify report offered load in flit/cycle/node, Michael Tan, 20260715
                ...
                injected_packets,
                received_packets,
                received_flits_in_window,
                received_packets_in_window,
                flit_throughput_x1000000,
                packet_throughput_x1000000,
                max_source_queue_occupancy,
                average_latency_x1000,
                error_seen);

        $display("[TB_THROUGHPUT_SWEEP] rate=%0d received_flits_window=%0d received_packets_window=%0d flit_throughput_x1000000=%0d packet_throughput_x1000000=%0d injected=%0d received=%0d queue_full=%0d max_q=%0d errors=%0d",
                 current_rate_permille, received_flits_in_window, received_packets_in_window,
                 flit_throughput_x1000000, packet_throughput_x1000000,
                 injected_packets, received_packets, blocked_packets, max_source_queue_occupancy, error_seen);//Modify log fixed-window normalized throughput, Michael Tan, 20260715
