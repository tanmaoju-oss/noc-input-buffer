//Modify add independent four-VC 5x5 4-flit center-hotspot latency sweep tb, Michael Tan, 20260722
`timescale 1ns / 1ps

import noc_params::*;

module tb_mesh_injection_sweep_5x5_noxim_queue_knee_4flit_4vc_hotspot #(
    ...
    parameter DRAIN_CYCLES_PER_RATE = 16000,//Modify extend drain for the single hotspot's 4-flit local-output bottleneck, Michael Tan, 20260722
    ...
    parameter HOTSPOT_X = 2,//Modify place the single hotspot at the 5x5 mesh center, Michael Tan, 20260722
    parameter HOTSPOT_Y = 2,//Modify place the single hotspot at the 5x5 mesh center, Michael Tan, 20260722
    parameter HOTSPOT_PROBABILITY_PERMILLE = 200,//Modify direct 20 percent of non-hotspot-source packets to the hotspot, Michael Tan, 20260722
    ...
    int hotspot_packets;//Modify count measurement packets explicitly directed to the center hotspot, Michael Tan, 20260722
    ...
    begin
        ...
        if(HOTSPOT_X < 0 || HOTSPOT_X >= MESH_SIZE_X || HOTSPOT_Y < 0 || HOTSPOT_Y >= MESH_SIZE_Y)
            $fatal(1, "[TB_HOTSPOT] hotspot (%0d,%0d) is outside the %0dx%0d mesh", HOTSPOT_X, HOTSPOT_Y, MESH_SIZE_X, MESH_SIZE_Y);//Modify reject an invalid hotspot coordinate, Michael Tan, 20260722
        if(HOTSPOT_PROBABILITY_PERMILLE < 0 || HOTSPOT_PROBABILITY_PERMILLE > 1000)
            $fatal(1, "[TB_HOTSPOT] hotspot probability must be 0..1000 permille, got %0d", HOTSPOT_PROBABILITY_PERMILLE);//Modify reject an invalid hotspot probability, Michael Tan, 20260722
        dump_output();
        initialize_global();
        open_result_file();
    ...
    task dump_output();
        $dumpfile("out.vcd");
        $dumpvars(0, tb_mesh_injection_sweep_5x5_noxim_queue_knee_4flit_4vc_hotspot);//Modify use independent four-VC center-hotspot sweep top, Michael Tan, 20260722
    endtask
    ...
    task open_result_file();
        ...
        $fwrite(result_fd, "injection_rate_permille injection_rate warmup_cycles measure_cycles drain_limit_cycles drain_used_cycles measure_generated measure_enqueued measure_queue_full measure_injected measure_received max_source_queue avg_latency_cycles_x1000 error_count hotspot_x hotspot_y hotspot_probability_permille measure_hotspot_packets measured_hotspot_share_x1000000\n");//Modify add hotspot configuration and measured traffic-share statistics, Michael Tan, 20260722
    endtask
    ...
    task reset_for_rate(input int rate_permille);
        ...
        hotspot_packets = 0;//Modify reset measured center-hotspot packet count per injection rate, Michael Tan, 20260722
        cycle_count = 0;
    ...
    task create_packet(input int src_x, input int src_y);
        ...
        if(current_phase == PHASE_MEASURE && dst_x == HOTSPOT_X && dst_y == HOTSPOT_Y)
            hotspot_packets++;//Modify count only measurement-window packets directed to the hotspot, Michael Tan, 20260722
    ...
    task choose_destination(input int src_x, input int src_y, output int dst_x, output int dst_y);
        if((src_x != HOTSPOT_X || src_y != HOTSPOT_Y) &&
           ($urandom_range(0, 999) < HOTSPOT_PROBABILITY_PERMILLE))
        begin
            dst_x = HOTSPOT_X;
            dst_y = HOTSPOT_Y;//Modify direct H=0.2 of non-hotspot-source packets to center node (2,2), Michael Tan, 20260722
        end
        else
        begin
            do
            ...
            while((dst_x == src_x && dst_y == src_y) ||
                  (dst_x == HOTSPOT_X && dst_y == HOTSPOT_Y));//Modify keep the random branch non-self and non-hotspot so H remains exactly 0.2, Michael Tan, 20260722
        end
    endtask
    ...
    task write_one_rate_result();
        ...
        longint hotspot_share_x1000000;//Modify report the realized hotspot fraction for traffic-model verification, Michael Tan, 20260722
        ...
        hotspot_share_x1000000 = 0;
        if(attempted_packets > 0)
            hotspot_share_x1000000 = (hotspot_packets * 1000000) / attempted_packets;//Modify normalize hotspot packets over all measurement-generated packets, Michael Tan, 20260722

        $fwrite(result_fd, "%0d %0d.%03d %0d %0d %0d %0d %0d %0d %0d %0d %0d %0d %0d %0d %0d %0d %0d %0d %0d\n",
                ...
                HOTSPOT_X,
                HOTSPOT_Y,
                HOTSPOT_PROBABILITY_PERMILLE,
                hotspot_packets,
                hotspot_share_x1000000);//Modify persist hotspot settings and realized measurement share, Michael Tan, 20260722
        ...
        $display("[TB_HOTSPOT] hotspot=(%0d,%0d) probability_permille=%0d measured_packets=%0d measured_share_x1000000=%0d",
                 HOTSPOT_X, HOTSPOT_Y, HOTSPOT_PROBABILITY_PERMILLE, hotspot_packets, hotspot_share_x1000000);//Modify log realized center-hotspot traffic share, Michael Tan, 20260722
