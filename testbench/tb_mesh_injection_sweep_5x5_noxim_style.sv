...
module tb_mesh_injection_sweep_5x5_noxim_style #(
    ...
    parameter DRAIN_CYCLES_PER_RATE = 3000,//Modify use bounded drain window for complete Vivado runs, Michael Tan, 20260629
    ...
    begin
        ...
        $fclose(result_fd);
        $display("[TB_INJ_SWEEP_NOXIM] results written to injection_latency_results.txt");//Modify identify Noxim-style sweep log, Michael Tan, 20260629
        $finish;
    end
    ...
    task dump_output();
        $dumpfile("out.vcd");
        $dumpvars(0, tb_mesh_injection_sweep_5x5_noxim_style);//Modify use independent Noxim-style 5x5 sweep top, Michael Tan, 20260629
    endtask
    ...
    task open_result_file();
        ...
        if(result_fd == 0)
        begin
            $fatal(1, "[TB_INJ_SWEEP_NOXIM] cannot open injection_latency_results.txt");//Modify identify Noxim-style sweep fatal, Michael Tan, 20260629
        end

        $fwrite(result_fd, "injection_rate_permille injection_rate warmup_cycles measure_cycles drain_cycles measure_attempted measure_injected measure_blocked measure_received avg_latency_cycles_x1000 error_count\n");//Modify output Noxim-style window and measurement-only statistics, Michael Tan, 20260629
    endtask
    ...
    task run_one_rate(input int rate_permille);
        ...
        current_phase = PHASE_DRAIN;//Modify stop new packets after measurement and drain existing traffic, Michael Tan, 20260629
        drain_active_generators();//Modify finish already-started packets before output drain/statistics, Michael Tan, 20260622

        @(posedge clk);
        cycle_count = WARMUP_CYCLES_PER_RATE + MEASURE_CYCLES_PER_RATE;//Modify keep absolute cycle count for latency, Michael Tan, 20260629
                    ...
                    begin
                        ...
                        begin
                            if(current_phase == PHASE_MEASURE)
                                attempted_packets++;//Modify count attempted packets only in measurement window, Michael Tan, 20260629
                            ...
                            begin
                                if(current_phase == PHASE_MEASURE)
                                    blocked_packets++;//Modify count blocked packets only in measurement window, Michael Tan, 20260629
                            end
                        end
                    end
    ...
    task start_packet(input int src_x, input int src_y);
        ...
        pkt_is_measured[pkt_id] = (current_phase == PHASE_MEASURE);//Modify mark packets injected during measurement window, Michael Tan, 20260629
        ...
        if(pkt_is_measured[pkt_id])
            injected_packets++;//Modify count injected packets only in measurement window, Michael Tan, 20260629
    endtask
                ...
                begin
                    ...
                    begin
                        ...
                        begin
                            ...
                            total_latency_cycles += (cycle_count - pkt_inject_cycle[pkt_id]);//Modify latency excludes warm-up injected packets, Michael Tan, 20260629
                        end
                    end
                    else
                    begin
    ...
    task write_one_rate_result();
        ...
        $fwrite(result_fd, "%0d 0.%0d %0d %0d %0d %0d %0d %0d %0d %0d %0d\n",
        ...
        $display("[TB_INJ_SWEEP_NOXIM] rate=%0d warmup=%0d measure=%0d drain=%0d attempted=%0d injected=%0d blocked=%0d received=%0d avg_latency_x1000=%0d errors=%0d",
        ...
        if(error_seen != 0)
            $error("[TB_INJ_SWEEP_NOXIM] rate=%0d observed %0d error_o assertions", current_rate_permille, error_seen);//Modify identify Noxim-style sweep error, Michael Tan, 20260629
        if(received_packets != injected_packets)
            $error("[TB_INJ_SWEEP_NOXIM] rate=%0d injected_packets=%0d but received_packets=%0d",
