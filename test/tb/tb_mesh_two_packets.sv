...
module tb_mesh_two_packets #(
    ...
    int num_op, dest_flits_seen, error_seen;
    flit_label_t expected_labels [4];
    flit_t flits_written [MESH_SIZE_X-1:0][MESH_SIZE_Y-1:0];
    ...
    task dump_output();
        $dumpfile("out.vcd");
        $dumpvars(0, tb_mesh_two_packets);
    endtask
    ...
    task initialize();
        ...
        expected_labels[0] = HEAD;
        expected_labels[1] = TAIL;
        expected_labels[2] = HEAD;
        expected_labels[3] = TAIL;
    ...
    task test_two_back_to_back_packets();
    ...
    task monitor_outputs();
        ...
        begin
            ...
            begin
                if(is_valid_o[x][y])
                begin
                    $display("[TB_MESH_TWO_PACKETS] output flit %0d at (%0d,%0d): label=%0d vc=%0d time=%0t",
                             dest_flits_seen, x, y, data_o[x][y].flit_label, data_o[x][y].vc_id, $time);

                    if(x != x_dest || y != y_dest)
                    begin
                        $error("[TB_MESH_TWO_PACKETS] flit arrived at wrong node (%0d,%0d), expected (%0d,%0d)",
                               x, y, x_dest, y_dest);
                    end

                    if(dest_flits_seen < 4)
                    begin
                        if(data_o[x][y].flit_label !== expected_labels[dest_flits_seen])
                        begin
                            $error("[TB_MESH_TWO_PACKETS] flit %0d label mismatch: got %0d expected %0d",
                                   dest_flits_seen, data_o[x][y].flit_label, expected_labels[dest_flits_seen]);
                        end

                        if(expected_labels[dest_flits_seen] == HEAD)
                        begin
                            if(data_o[x][y].data.head_data.x_dest !== x_dest ||
                               data_o[x][y].data.head_data.y_dest !== y_dest)
                            begin
                                $error("[TB_MESH_TWO_PACKETS] HEAD flit %0d destination fields are wrong",
                                       dest_flits_seen);
                            end
                        end
                    end
                    ...
                    begin
                        $error("[TB_MESH_TWO_PACKETS] unexpected extra output flit");
                    end
                ...
                begin
                    ...
                    begin
                        ...
                        begin
                            error_seen++;
                            $error("[TB_MESH_TWO_PACKETS] error_o asserted at node=(%0d,%0d) port=%0d vc=%0d time=%0t",
                                   x, y, port, vc, $time);
                        end
                    end
                end
    ...
    task final_checks();
        if(dest_flits_seen != 4)
            $error("[TB_MESH_TWO_PACKETS] expected 4 output flits at destination, saw %0d", dest_flits_seen);
        if(error_seen != 0)
            $error("[TB_MESH_TWO_PACKETS] observed %0d error_o assertions", error_seen);
        if(dest_flits_seen == 4 && error_seen == 0)
            $display("[TB_MESH_TWO_PACKETS] PASSED");
    endtask

    task create_flit(input int x, input int y, input flit_label_t lab, input int vc_num);
        flits_written[x][y].flit_label = lab;
        flits_written[x][y].vc_id = vc_num[VC_SIZE-1:0];
        if(lab == HEAD || lab == HEADTAIL)
        begin
            flits_written[x][y].data.head_data.x_dest = x_dest;
            flits_written[x][y].data.head_data.y_dest = y_dest;
            flits_written[x][y].data.head_data.head_pl = num_op[HEAD_PAYLOAD_SIZE-1:0];
        end
        else
        begin
            flits_written[x][y].data.bt_pl = num_op[FLIT_DATA_SIZE-1:0];
        end
    endtask

    task write_flit(input int x, input int y, input flit_label_t lab, input int vc_num);
