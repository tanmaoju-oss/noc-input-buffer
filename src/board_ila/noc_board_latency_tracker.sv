...
module noc_board_latency_tracker #(
    ...
    (* ram_style = "block" *) logic [COUNTER_WIDTH-1:0] enqueue_cycle_mem [0:TRACK_TABLE_DEPTH-1];//Modify infer one 2048-by-32 BRAM per source tracker, Michael Tan, 20260909
    ...
    logic [COUNTER_WIDTH-1:0] tail_enqueue_cycle_q;
    ...
    assign tail_enqueue_cycle_o = tail_enqueue_cycle_q;
    ...
    always_ff @(posedge clk) begin : per_source_tracker
        ...
        if (rst) begin
            ...
            if (enqueue_event_i) begin
                ...
                enqueue_cycle_mem[enqueue_sequence_i[TRACK_INDEX_WIDTH-1:0]] <= enqueue_cycle_i;
            end
            ...
            if (tail_fifo_count_q != 0) begin
                ...
                tail_enqueue_cycle_q <= enqueue_cycle_mem[tail_fifo_sequence[tail_fifo_head_q][TRACK_INDEX_WIDTH-1:0]];//Modify capture synchronous BRAM read data for the following monitor clock, Michael Tan, 20260909
