...
# Modify add WSL/Linux Vivado entry for the 0.11-to-0.20 performance-TB comparison sweep, Michael Tan, 20260908
set -euo pipefail
...
TOP="tb_mesh_injection_sweep_5x5_noxim_queue_knee_4flit_4vc_rate_011_to_020"
...
if [[ "$(awk 'NR > 1 {count++} END {print count+0}' injection_latency_results.txt)" -ne 10 ]]; then
    echo "ERROR: Expected 10 completed injection-rate rows" >&2 #Modify require all 0.11-to-0.20 comparison points, Michael Tan, 20260908
    exit 1
fi
