 ...
 *   2. 支持多个数据包在同一VC中排队（通过元数据FIFO），独立于缓冲区，将同方向的packet按照packet进行传输
...
module input_buffer #(
    ...
    } packet_meta_t;                          // 目的是存packet，存两个

    // 元数据指针位宽计算
    localparam [31:0] META_POINTER_SIZE = $clog2(BUFFER_SIZE);

    // ========== 信号声明 ==========
    flit_novc_t first_flit_novc;  // 第一个flit（未使用VC，由Michael Tan添加，20260528）//无用，可删
    ...
    flit_novc_t read_flit;        // 从循环缓冲区读出的flit，来自于noc参数中
    ...
    begin
        ...
        data_o.flit_label = 00.flit_label;
